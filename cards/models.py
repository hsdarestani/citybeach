...
def member_number(): return str(secrets.randbelow(90_000_000)+10_000_000)
def receipt_number(): return f"CB-{timezone.now():%Y%m%d}-{uuid.uuid4().hex[:8].upper()}"
...
class BusinessSettings(models.Model):
    ...
    @property
    def tip_options(self): return [self.tip_1,self.tip_2,self.tip_3,self.tip_4]
...
class Wallet(models.Model):
    ...
    def __str__(self): return f'{self.member_number} · {self.display_name}'
...
class PaymentRequest(models.Model):
    ...
    @property
    def total(self): return self.base_amount+self.tip_amount
...
class Offer(models.Model):
    class Target(models.TextChoices):
        ALL='ALL','Alle'
        BEACH='BEACH','Beach Member'
        SUNSET='SUNSET','Sunset Member'
        SKYLINE='SKYLINE','Skyline Member'
